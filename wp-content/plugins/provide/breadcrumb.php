...
class provide_WP_Breadcrumb {
	...
	public function __construct( $templates = array(), $options = array(), $strings = array(), $autorun = TRUE ) {
		...
		$this->strings   = wp_parse_args( $strings, array(
			'home'      => esc_html__( 'Home', 'provide' ),
			'_search'   => array(
				'_singular' => esc_html__( 'A search result to', 'provide' ) . ' <em>%s</em>',
				'_plural'   => '%s ' . esc_html__( 'Search Results on', 'provide' ) . ' <em>%s</em>'
			),
			'paged'     => esc_html__( 'Page', 'provide' ) . ' %d',
			'404_error' => esc_html__( 'Error: page does not exist', 'provide' )
		) );
		// Generate breadcrumb
		if ( $autorun ) {
			echo $this->output();
	...
	protected function template( $item, $type = 'standard' ) {
		...
		switch ( $type ) {
			case'link':
				return $this->template( sprintf( $this->templates[ 'link' ], esc_url( $this->h->provide_set( $item, 'link' ) ), $this->h->provide_set( $item, 'title' ) ) );
				break;
			case 'current':
	...
	public function generate() {
		...
		if ( is_singular() ) { // Posts, (Sub)Pages, Attachments and Custom Post Types
			...
			$text                          = sprintf( _n( $this->strings[ '_search' ][ '_singular' ], $this->strings[ '_search' ][ '_plural' ], $total ), $total, get_search_query() );
			$this->breadcrumb[ '_search' ] = $this->template( $text, 'current' );
			if ( $this->options[ 'show_pagenum' ] ) {
				$this->breadcrumb[ '_search' ] = $this->template( array(
			...
			if ( is_category() || is_tag() || is_tax() ) { // Categories, Tags and Custom Taxonomies
				...
				if ( is_year() ) { // Year archive
					...
					$this->breadcrumb[ 'archive_day' ]   = $this->template( array( 'title' => get_the_date( 'j' ) ) );
					if ( $this->options[ 'show_pagenum' ] ) {
						$this->breadcrumb[ 'archive_day' ] = $this->template( array(
							'link'  => get_month_link( get_query_var( 'year' ), get_query_var( 'monthnum' ), get_query_var( 'day' ) ),
							'title' => get_the_date( 'F' )
						) );
					}
				}
		...
		if ( $this->options[ 'show_pagenum' ] ) {
			$this->breadcrumb[ 'paged' ] = $this->template( sprintf( $this->strings[ 'paged' ], get_query_var( 'paged' ) ), 'current' );
		}
	}
}
