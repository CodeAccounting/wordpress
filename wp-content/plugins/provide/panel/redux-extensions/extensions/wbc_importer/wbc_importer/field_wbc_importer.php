			...
			if ( ! empty( $this->field['wbc_demo_imports'] ) ) {
				foreach ( $this->field['wbc_demo_imports'] as $section => $imports ) {
					...
					if ( ! array_key_exists( 'imported', $imports ) ) {
						...
						$imported       = false;
						$import_message = esc_html__( 'Import Demo', 'provide' );
					} else {
						$imported       = true;
						$extra_class    = 'active imported';
						$import_message = esc_html__( 'Demo Imported', 'provide' );
					}
					...
					echo '<h3 class="theme-name">' . esc_html( apply_filters( 'wbc_importer_directory_title', ucwords( str_replace( '_', ' ', $imports['directory'] ) ) ) ) . '</h3>';
					echo '<div id="' . $imports['directory'] . '" class="theme-actions">';
					if ( false == $imported ) {
						echo '<div class="wbc-importer-buttons"><span class="spinner">' . esc_html__( 'Please Wait...', 'provide' ) . '</span><span class="button-primary importer-button import-demo-data">' . esc_html__( 'Import Demo', 'provide' ) . '</span></div>';
					} else {
						echo '<div class="wbc-importer-buttons button-secondary importer-button">' . esc_html__( 'Imported', 'provide' ) . '</div>';
						echo '<span class="spinner">' . esc_html__( 'Please Wait...', 'provide' ) . '</span>';
						echo '<div id="wbc-importer-reimport" class="wbc-importer-buttons button-primary import-demo-data importer-button">' . esc_html__( 'Re-Import', 'provide' ) . '</div>';
					}
					echo '</div>';
					echo '</div>';
				...
				echo "<h5>" . esc_html__( 'No Demo Data Provided', 'provide' ) . "</h5>";
			}
			echo '</div></div>';
			echo '</fieldset></td></tr>';
		}
