...
if ( ! class_exists( 'ReduxFramework_extension_wbc_importer' ) ) {
	class ReduxFramework_extension_wbc_importer {
		...
		public function ajax_importer() {
			...
			// check rev slider is active or not
			if ( ! function_exists( 'rev_slider_shortcode' ) ) {
				echo esc_html__( 'Please Active Revolution Slider Plugin From Plugin Menu', 'provide' );
				exit;
			}

			// check visual composer is active or not
			if ( ! function_exists( 'vc_map' ) ) {
				echo esc_html__( 'Please Active Visual Composer Plugin From Plugin Menu', 'provide' );
				exit;
			}
			if ( isset( $_REQUEST['type'] ) && $_REQUEST['type'] == "import-demo-content" && array_key_exists( $_REQUEST['demo_import_id'], $this->wbc_import_files ) ) {
				...
				if ( ! isset( $import_parts['imported'] ) || true === $reimporting ) {
					...
					include_once Plugin_ROOT . 'import_export.php';
					$themeSettings = new provide_import_export( $_REQUEST['demo'] );
					$themeSettings->provide_import();
				} else {
					echo esc_html__( "Demo Already Imported", 'provide' );
				}
				die();
			}
			die();
		...
		function add_importer_section() {
			...
			$wbc_importer_label       = trim( esc_html( apply_filters( 'wbc_importer_label', esc_html__( 'Demo Importer', 'provide' ) ) ) );
			$wbc_importer_label       = ( ! empty( $wbc_importer_label ) ) ? $wbc_importer_label : esc_html__( 'Demo Importer', 'provide' );
			$this->parent->sections[] = array(
				'id'     => 'wbc_importer_section',
				'title'  => $wbc_importer_label,
				'desc'   => '<p class="description">' . apply_filters( 'wbc_importer_description', esc_html__( 'Works best to import on a new install of WordPress', 'provide' ) ) . '</p>',
