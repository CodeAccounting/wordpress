(function ($) {
    ...
    redux.field_objects.language.deleteFile = function (el) {
        $('a#delete-mo-file').on('click', function () {
            ...
            if (file != '') {
                var data = 'file=' + file + '&action=provide_deleteLanguageFile';
                $.ajax({
                    type: 'POST',
                    url: ajaxurl,
                    dataType: 'json',
    ...
    redux.field_objects.language.uploader = function () {
        if (window.File && window.FileReader && window.FileList && window.Blob) {
            ...
            var ftype = $('input[type=file]')[0].files[0].name;
            ...
            formData.append('action', 'provide_languageUpload');
            formData.append('language_file', $('input[type=file]')[0].files[0]);
            $("#error-log").empty();
            $.ajax({
                url: ajaxurl,
                type: "POST",
