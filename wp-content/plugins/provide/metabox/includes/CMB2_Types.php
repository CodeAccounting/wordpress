...
class CMB2_Types
{
    ...
    public function concat_items($args = array())
    {
        ...
        $value = $this->field->escaped_value() ? $this->field->escaped_value() : $this->field->args('default');
    ...
    public function render_repeatable_field()
    {
        ...
        <p class="cmb-add-row">
            <button type="button" data-selector="<?php echo $table_id; ?>" class="cmb-add-row-button button"><?php echo esc_html($this->_text('add_row_text', __('Add Row', 'provide'))); ?></button>
        </p>
    ...
    protected function repeat_row($disable_remover = false, $class = 'cmb-repeat-row')
    {
        ...
        <div class="cmb-row <?php echo $class; ?>">
            ...
            <div class="cmb-td cmb-remove-row">
                <button type="button" class="button cmb-remove-row-button<?php echo $disabled; ?>"><?php echo esc_html($this->_text('remove_row_text', __('Remove', 'provide'))); ?></button>
            </div>
        </div>
    ...
    public function iconbox($args = array())
    {
        $a = $this->parse_args($args, 'textarea', array(
            'name' => $this->_name(),
            'id' => $this->_id(),
        ...
        ob_start();
        $icons = provide_IconBoxIcons();
        echo sprintf('<input type="hidden"%s/>', $this->concat_attrs($a, array('desc', 'js_dependencies')));
        ?>
        <div class="icons-pack" id="<?php echo self::$counter ?>">
            <div class="icons-sec" id="iconbox<?php echo self::$counter ?>">
                <ul>
                    <?php
                    if (!empty($icons) && count($icons) > 0) {
                        foreach ($icons as $icon) {
                            $selected = ($a['value'] == $icon) ? 'class="active"' : '';
                            echo '<li ' . $selected . '><i class="' . $icon . '"></i></li>';
                        }
                    }
                    ?>
                </ul>
        ...
        <script type="text/javascript">
            jQuery(document).ready(function ($) {
                $('div.icons-pack > #iconbox<?php echo esc_js(self::$counter) ?>').enscroll();
            });
        </script>
        <?php
        self::$counter++;
    ...
    public function radio_img($args = array(), $type = 'radio')
    {
        $a = $this->parse_args($args, $type, array(
            'class' => 'cmb2-radio-list-img',
            'options' => $this->concat_items(array('label' => 'test', 'method' => 'list_input')),
            'desc' => $this->_desc(true),
        ));
    ...
    public function checkbox($args = array(), $is_checked = null)
    {
        ...
        $is_checked = null === $is_checked ? !empty($meta_value) : $is_checked;
        ...
        return sprintf('<div class="onoffswitch">%s<label class="onoffswitch-label" for="%s"><span class="onoffswitch-inner"></span><span class="onoffswitch-switch"></span></label></div>', $this->input($args), $this->_id(), $this->_id());
    }
    ...
    public function taxonomy_radio()
    {
        ...
        if (!$terms) {
            $options .= sprintf('<li><label>%s</label></li>', esc_html($this->_text('no_terms_text', __('No terms', 'provide'))));
        } else {
            $option_none = $this->field->args('show_option_none');
            if (!empty($option_none)) {
    ...
    public function oembed()
    {
        ...
        $oembed = !empty($meta_value) ? cmb2_get_oembed(array(
    ...
    public function file_list()
    {
        ...
        $this->input(array(
            ...
            'value' => esc_html($this->_text('add_upload_files_text', __('Add or Upload Files', 'provide'))),
            'name' => '', 'id' => '',
        ));
    ...
    public function file()
    {
        ...
        printf('<input class="cmb2-upload-button button" type="button" value="%s" />', esc_attr($this->_text('add_upload_file_text', __('Add or Upload File', 'provide'))));
    ...
    {
        printf('<%1$s class="img-status">%2$s<p class="cmb2-remove-wrapper"><a href="#" class="cmb2-remove-file-button"%3$s>%4$s</a></p>%5$s</%1$s>', $args['tag'], $args['image'], isset($args['cached_id']) ? ' rel="' . $args['cached_id'] . '"' : '', esc_html($this->_text('remove_image_text', __('Remove Image', 'provide'))), isset($args['id_input']) ? $args['id_input'] : ''
        );
    }
    ...
    {
        printf('<%1$s class="file-status"><span>%2$s <strong>%3$s</strong></span>&nbsp;&nbsp; (<a href="%4$s" target="_blank" rel="external">%5$s</a> / <a href="#" class="cmb2-remove-file-button"%6$s>%7$s</a>)%8$s</%1$s>', $args['tag'], esc_html($this->_text('file_text', __('File:', 'provide'))), $this->get_file_name_from_path($args['value']), $args['value'], esc_html($this->_text('file_download_text', __('Download', 'provide'))), isset($args['cached_id']) ? ' rel="' . $args['cached_id'] . '"' : '', esc_html($this->_text('remove_text', __('Remove', 'provide'))), isset($args['id_input']) ? $args['id_input'] : ''
        );
    }
