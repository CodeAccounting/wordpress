...
if ( !class_exists( 'provide_Resizer' ) ) {
	...
	class provide_Resizer {

		public function provide_resize( $url, $width = null, $height = null, $crop = true, $align = 'c', $retina = false, $isUrl ) {
			global $wpdb;
			$common = $this->provide_info( $url, $width, $height, $crop, $align, $retina );
			if ( is_array( $common ) ) {
				extract( $common );
			} else {
				return $common;
			...
			if ( $isUrl === true ) {
				...
				return '<img width="' . $width . '" height="' . $height . '" src="' . $resized_url . '" alt=""/>';
			}
		}

		public function provide_info( $url, $width, $height, $crop, $align, $retina ) {
			if ( empty( $url ) ) {
		...
		public function provide_delResizer( $post_id ) {
			...
			$wp_upload_dir = wp_upload_dir();
			$pathinfo = pathprovide_info( $metadata['file'] );
