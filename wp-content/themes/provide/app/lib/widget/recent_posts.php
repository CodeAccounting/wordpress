...
class provide_recent_posts_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		$control_ops = array(
			'width'   => 250,
			'height'  => 350,
			'id_base' => 'provide-recent-posts'
		);
		parent::__construct( 'provide-recent-posts', esc_html__( 'Latest News - Provide', 'provide' ), $widget_ops, $control_ops );
	}

	public function widget( $args, $instance ) {
		extract( $args );
		$defaults = array( 'title' => esc_html__( 'LATEST NEWS', 'provide' ), 'number' => '3', 'category' => '', 'title_limit' => '25' );
		...
		$sizes = array( 'm' => '93x59', 'i' => '93x59', 'w' => '93x59' );
					...
					<div class="news-detail">
						<h5>
							<a href="<?php the_permalink() ?>" title="<?php the_title() ?>">
								<?php echo substr( get_the_title(), 0, (int)$limit ) ?>
							</a>
						</h5>
						<span><?php $this->h->provide_date( true, 'M d - Y' ) ?></span>
					</div>
				</div>
				<?php
			}
