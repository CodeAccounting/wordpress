...
if (! function_exists('et_multisite_thumbnail')) {
    function et_multisite_thumbnail($thumbnail = '')
    {
        ...
        if (isset($matches[2])) {
            ...
            if (is_file($file)) {
                $thumbnail = str_replace(ABSPATH, trailingslashit(get_site_url(1)), $file);
            } else {
                $thumbnail = '';
            }
        }
...
if (! function_exists('show_page_menu')) {
    function show_page_menu($customClass = 'nav clearfix', $addUlContainer = true, $addHomeLink = true)
    {
        ...
        if (et_get_option($shortname . '_home_link') == 'on' && $addHomeLink) {
            ?>
				<li <?php if (is_front_page() || is_home()) {
                echo 'class="current_page_item"';
            } ?>><a href="<?php echo esc_url(home_url()); ?>"><?php esc_html_e('Home', $themename); ?></a></li>
			<?php
...
if (! function_exists('show_categories_menu')) {
    function show_categories_menu($customClass = 'nav clearfix', $addUlContainer = true)
    {
        ...
        //excluded categories
        if (et_get_option($shortname.'_menucats') <> '') {
            $exclude_cats = implode(",", et_get_option($shortname.'_menucats'));
        }

        //hide empty categories
        if (et_get_option($shortname.'_categories_empty') == 'on') {
            $hide = '1';
        } else {
            $hide = '0';
        }
        ...
        $strdepth2 = '';
        if (et_get_option($shortname.'_enable_dropdowns_categories') == 'on') {
            $strdepth2 = "depth=".et_get_option($shortname.'_tiers_shown_categories');
        }
        if ($strdepth2 == '') {
            $strdepth2 = "depth=1";
        }
...
function head_addons()
{
    ...
    if (et_get_option($shortname . '_custom_colors') == 'on') {
        et_epanel_custom_colors_css();
    }
    ...
    function elegant_keywords()
    {
        ...
        if (et_get_option($shortname.'_seo_single_keywords') == 'on') {
            ...
            if (isset($exists4) && $exists4 !== '') {
                if (is_single() || is_page() || elegant_is_blog_posts_page()) {
                    echo '<meta name="keywords" content="' . esc_attr($exists4) . '" />';
                }
            }
        }
    }
    ...
    function elegant_canonical()
    {
        ...
        if (et_get_option($shortname.'_seo_index_canonical') == 'on') {
            ...
            if (is_archive() || is_category() || is_search()) {
                echo '<link rel="canonical" href="'. esc_url($current_page_url).'" />';
            }
        }
    }
}
...
if (! function_exists('et_resize_image')) {
    function et_resize_image($thumb, $new_width, $new_height, $crop)
    {
        ...
        if (!$size) {
            return new WP_Error('invalid_image_path', esc_html__('Image doesn\'t exist'), $thumb);
        }
...
function et_custom_posts_per_page($query = false)
{
    ...
    if ($query->is_category) {
        ...
        if (isset($_GET['et_searchform_submit'])) {
            $postTypes = array();
            if (!isset($_GET['et-inc-posts']) && !isset($_GET['et-inc-pages'])) {
                $postTypes = array('post');
            }
            if (isset($_GET['et-inc-pages'])) {
                $postTypes = array('page');
            }
            if (isset($_GET['et-inc-posts'])) {
                $postTypes[] = 'post';
            }
...
if (version_compare(phpversion(), '4.4', '>=')) {
    add_filter('get_comments_number', 'et_comment_count', 0, 2);
}
...
function et_add_fullwidth_body_class($classes)
{
    ...
    if (is_single() && 'on' == get_post_meta(get_queried_object_id(), '_et_full_post', true)) {
        $fullwidth_view = true;
    }
...
function et_add_custom_css()
{
    ...
    if (false === $custom_css || '' == $custom_css) {
        return;
    }
